...
echo "3 Testing CompletableFuture aggregate:"
curl -s http://localhost:8080/aggregate-old | head -1
echo ""

echo "4 Testing first success:"
curl -s http://localhost:8080/first-success | head -1
echo ""

echo "5 Testing aggregate with fallback:"
curl -s http://localhost:8080/aggregate-with-fallback | head -1
echo ""

echo "6 Testing multi-service aggregation:"
curl -s http://localhost:8080/multi-aggregate | head -1
echo ""

echo "7 Performance testing with wrk:"
echo "wrk -t4 -c100 -d10s http://localhost:8080/aggregate"
echo "wrk -t4 -c100 -d10s http://localhost:8080/aggregate-old"